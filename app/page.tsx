...
export default function Home() {
  ...
  useEffect(() => {
    ...
    ]).then(([k, m, l]) => { setKpis(k); setMarcas(Array.isArray(m) ? m : []); setLojas(Array.isArray(l) ? l : []); setLoading(false) })
      .catch(() => setLoading(false))
  }, [])

  const fmt  = (n: number) => n != null ? Number(n).toLocaleString("pt-BR") : "0"
  // Formato compacto para valores grandes — evita estourar o card
            ...
            <tbody>
              {lojas.map((l: any, i: number) => (
                <tr key={i} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td style={{ padding: "12px 14px", color: "var(--primary)", fontWeight: 600, whiteSpace: "nowrap" }}>{fmtR(l.valor_estoque)}</td>
